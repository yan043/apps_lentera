                                        ...
                                        <tr>
                                            ...
                                            <td style="width: 11%">
                                                ...
                                                <div class="modal fade" id="editModal{{ $v->id }}" tabindex="-1" role="dialog" aria-hidden="true">
                                                    <div class="modal-dialog modal-dialog-centered modal-sm">
                                                        <div class="modal-content">
                                                            <form action="{{ route('settings.level.store') }}" method="POST">
                                                                @csrf
                                                                @method('POST')
                                                                <input type="hidden" name="id" value="{{ $v->id }}">
                                                                <div class="modal-header">
                                                                    <h4 class="modal-title" id="editModal{{ $v->id }}Label">Edit Level</h4>
                                                ...
                                                <div class="modal fade" id="deleteModal{{ $v->id }}" tabindex="-1" role="dialog" aria-hidden="true">
                                                    <div class="modal-dialog modal-dialog-centered modal-sm">
                                                        <div class="modal-content">
                                                            <form action="{{ route('settings.level.destroy', $v->id) }}" method="POST">
                                                                @csrf
                                                                @method('DELETE')
                                                                <div class="modal-header">
                                                                    <h4 class="modal-title" id="deleteModal{{ $v->id }}Label">Delete Level</h4>
                                                                    <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span></button>
                                                                </div>
                                                                <div class="modal-body">
