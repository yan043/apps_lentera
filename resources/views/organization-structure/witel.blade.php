				...
				<div class="modal-body">
					<form action="/organization-structure/witel/store" method="POST">
						...
						<div class="mb-3">
							...
							<input type="text" class="form-control" name="scope" placeholder="Enter Scope" required>
						</div>
...
@section('scripts')
	...
	<script>
				...
				columns: [{
					...
					},
					{
						...
						render: function(data, type, row) {
							return `
                            <button type="button" class="btn btn-sm btn-primary" onclick="openEditModal(${data}, '${row.regional_id}', '${row.name}', '${row.alias}', '${row.scope}')">
		...
		function openEditModal(id, regionalId, name, alias, scope) {
			...
			$('#modal-edit input[name="scope"]').val(scope);
