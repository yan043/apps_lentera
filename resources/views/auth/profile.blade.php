				...
				<div class="card-body">
					<div class="d-flex justify-content-center align-items-center flex-column">
						<div class="avatar avatar-lg">
							<img src="/assets/images/{{ $data->gender }}.png" alt="Avatar" style="width: 100px; height: 100px;" />
						</div>
							...
							<div class="col-md-6">
								<div class="mb-3">
									...
									<input type="text" name="nik" id="nik" class="form-control" placeholder="Masukan NIK"
										oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');" minlength="6"
						...
						<div class="row">
							<div class="col-md-6">
								<div class="mb-3">
									...
									<input type="text" name="chat_id" id="chat_id" class="form-control" placeholder="Masukan Chat ID Telegram"
										value="{{ $data->chat_id }}" />
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									...
									<input type="text" name="number_phone" id="number_phone" class="form-control"
										...
										oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');" minlength="11"
						...
						<div class="row">
							<div class="col-md-6">
								<div class="mb-3">
									...
									<select name="gender" id="gender" class="form-control select2" placeholder="Pilih Jenis Kelamin">
							...
							<div class="col-md-6">
								<div class="mb-3">
									...
									<input type="text" name="date_of_birth" id="date_of_birth" class="form-control datepicker"
										placeholder="Masukan Tanggal Lahir" value="{{ $data->date_of_birth }}" />
								</div>
							</div>
						</div>
						<div class="row">
								...
								<div class="mb-3">
									...
									<select name="regional_id" id="regional_id" class="form-control select2" required>
										...
										@foreach ($get_regional as $regional)
											<option value="{{ $regional->id }}" {{ $regional->id == $data->regional_id ? 'selected' : '' }}>
								...
								<div class="mb-3">
									...
									<select name="witel_id" id="witel_id" class="form-control select2" required>
										...
										@foreach ($get_witel as $witel)
											<option value="{{ $witel->id }}" {{ $witel->id == $data->witel_id ? 'selected' : '' }}>
												{{ $witel->name }}
											</option>
										@endforeach
									</select>
								...
								<div class="mb-3">
									...
									<select name="mitra_id" id="mitra_id" class="form-control select2" required>
										...
										@foreach ($get_mitra as $mitra)
											<option value="{{ $mitra->id }}" {{ $mitra->id == $data->mitra_id ? 'selected' : '' }}>
												{{ $mitra->name }}
											</option>
										@endforeach
									</select>
								...
								<div class="mb-3">
									...
									<select name="sub_unit_id" id="sub_unit_id" class="form-control select2" required>
										...
										@foreach ($get_sub_unit as $sub_unit)
											<option value="{{ $sub_unit->id }}" {{ $sub_unit->id == $data->sub_unit_id ? 'selected' : '' }}>
								...
								<div class="mb-3">
									...
									<select name="sub_group_id" id="sub_group_id" class="form-control select2" required>
										...
										@foreach ($get_sub_group as $sub_group)
											<option value="{{ $sub_group->id }}" {{ $sub_group->id == $data->sub_group_id ? 'selected' : '' }}>
